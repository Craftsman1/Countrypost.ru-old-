<div class='content'>
	...
	<div id=login class='field <?=$result->d && $result->d->user_login && $result->e != -17  ? 'done' :'';?>'>
		...
		<div class='text-field'>
			<div>
				<input type='text' name="login" value="<?=$result->d ? $result->d->user_login :'';?>" />
			</div>
		</div>
	</div>
	<div class='field <?=$result->d && $result->d->user_password ? 'done' :'';?>' id='password_div'>
		...
		<div class='text-field'>
			<div>
				<input type='password' id="password" name="password" value="<?=$result->d ? $result->d->user_password :'';?>" />
			</div>
		</div>
	</div>
	<div id=email class='field <?=$result->d && $result->d->user_email && $result->e != -13 ? 'done' :'';?>' >
		...
		<div class='text-field'>
			<div>
				<input type='text' id=email name="email" value="<?=$result->d ? $result->d->user_email :'';?>" />
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	...
	$.ajax({
		  type: 'POST',
		  url: '/signup/checkRegFields',
		  data: $("#registration").serialize(),
		  success: function (data) { 
		  	var d = $.parseJSON(data);
		  	if( d.code==-18 && $('input#captchacode').val()=='') 
			{
			  	d.text = 'Текст на картинке не введен.';
		  	}
