<div class='table' id="lay2_block" style="width:400px; position:fixed; z-index: 1000; display:none;">
	<center>
		<h3 style="margin-top:0;margin-bottom:20px;">Добавить товар в корзину</h3>
	...
	<form class='admin-inside' id="orderForm" action="<?=$selfurl?>addProductManualAjax/" enctype="multipart/form-data" method="POST">
		<table>
			<tr>
				<td>Страна: *</td>
				<td>
					<input name="ocountry" type="hidden" value="<?=$Country->country_id?>" />
					<input type="text" readonly value="<?=$Country->country_name?>" />
			...
			<tr>
				...
				<td>
					<input checked type="radio" value="1" id="img1" name="img"><label for="img1"><input id="userfileimg" type="text" name="userfileimg" value="" maxlength="4096" style="width:205px"></label><br>
			...
			<tr class='last-row'>
				<td colspan='9'>
					<div>	
						<div class='submit'>
							<div>
								<input onclick="$('#lay2_block').fadeOut('slow');openImportPopup();" style='width:163px;' type='button' name="importExcel" value='Загрузить товары из Exсel' />
							</div>
						</div>
					</div>
					<div class='float'>	
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#orderForm').ajaxForm({
			...
			{
				$('#progressbar').hide();
				$('em').html('Товар не добавлен. Попробуйте еще раз.<br /><br />').show();
			}
		});
	});
