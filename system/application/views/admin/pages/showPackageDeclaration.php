<div class='content'>
	...
	<form class='card' id="declarationForm" action='<?= $selfurl ?>saveDeclaration/<?= $package->package_id ?>' method='POST'>
		...
		<label class='filling-declaration'>
			<input id="help" type="checkbox" <? if ($package->declaration_status == 'help') : ?>checked="checked"<? endif; ?>/>
			<span>Помочь заполнить декларацию ($<?=$config['price_for_declaration']->config_value?>)<em>Если вы не знаете, что в посылке, поставьте тут галочку. Мы заполним декларацию за вас. Стоимость $<?=$config['price_for_declaration']->config_value?></em></span>
			<br />
		</label>
		<label style="margin-top:0;" class='filling-declaration' for="">
			<input id="insurance" type="checkbox" <? if ($package->package_insurance) : ?>checked="checked"<? endif; ?>/>
			<span>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#help').change(function() {
			...
			else if (confirm('Отказаться от помощи партнера в заполнении декларации выбранной посылки?'))
			{
				window.location = '<?= $selfurl ?>removeDeclarationHelp/<?= $package->package_id ?>';	
			}			
		});
