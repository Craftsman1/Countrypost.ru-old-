<div class="offline_order_form" style='display:none;'>
	<div class='table' style="position:relative;">
		<div class='angle angle-lt'></div>
		<div class='angle angle-rt'></div>
		<div class='angle angle-lb'></div>
		<div class='angle angle-rb'></div>	
		<form class='admin-inside' action="<?= $selfurl ?>checkout" id="offlineOrderForm" method="POST">
			<input type='hidden' name="order_id" class="order_id" />
			<div class='new_order_box'>
				<div>
					<span class="label">Заказать из*:</span>
					<select id="country_from_offline" name="country_from" class="textbox" onchange="setCountryFrom(this.value)">
				...
				<div>
					...
					<select id="country_to_offline" name="country_to" class="textbox" onchange="setCountryTo(this.value)">
						<option value="0">выберите страну...</option>
						<? foreach ($countries as $country) : ?>
	...
	<form class='admin-inside' action="<?= $selfurl ?>addProductManualAjax" id="offlineItemForm" method="POST">
		...
		<input type='hidden' class="countryTo" />
...
<script type="text/javascript">
	$(function() {
		...
		$('div.offline_order').click(function() {
			$.fn.getOrder({
				orderType : "offline",
				forms : {
					order : "#offlineOrderForm",
					item : "#offlineItemForm",
				},
				fields : {
					country_from : {
						selector : "#country_from_offline",
						required : true
					},
					country_to : {
						selector : "#country_to_offline",
						required : true
					}
				},
				show : function() 
				{
					...
					$('h2#page_title').html('Добавление нового Offline заказа');
					$("div.offline_order_form").show('slow');
				}
			});
		});
	...
	$(function() {
		$('#orderForm').ajaxForm({
			...
			success: function(response)
			{
				...
				if (response)
				{
					error('top', 'Заказ не добавлен. Заполните все поля и сохраните заказ еще раз.');
				}
				else
				{
